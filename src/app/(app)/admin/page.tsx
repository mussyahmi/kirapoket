...
export default function AdminPage() {
  ...
  const currentTarget =
    mau < EARLY_ADOPTER_TARGET ? EARLY_ADOPTER_TARGET : FULL_MONETIZE_TARGET;
  const currentLabel =
    mau < EARLY_ADOPTER_TARGET ? "early adopter launch" : "full monetization";
  ...
  const handleSeedDemo = async () => {
    if (
      !confirm(
        "This will wipe all data for the demo account and replace it with fresh seed data. Continue?",
      )
    )
      return;
    setSeedingDemo(true);
            ...
            <div className="rounded-xl border border-border bg-card px-4 py-3 flex items-center justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-sm font-semibold mt-0.5">
                  {newestSignup
                    ? formatDistanceToNow(newestSignup.when, {
                        addSuffix: true,
                      })
              ...
              {newestSignup && (
                <p className="text-xs text-muted-foreground truncate text-right shrink-0 max-w-[45%]">
                  {newestSignup.user.displayName ??
                    newestSignup.user.email ??
                    "—"}
                </p>
              )}
            </div>
              ...
              {[0, 1, 2, 3].map((i) => (
                <div key={i} className="flex items-center gap-3 px-4 py-3">
                  ...
                  <div className="flex-1 min-w-0 space-y-2">
                    ...
                    />
                    <Skeleton
                      className={`h-2.5 ${["w-40", "w-48", "w-36", "w-44"][i]}`}
                    />
                              ...
                              {(stats.memberSince ||
                                ...
                                <p className="text-xs text-muted-foreground">
                                  {stats.memberSince && (
                                    <>
                                      Member since{" "}
                                      {formatDistanceToNow(stats.memberSince, {
                                        addSuffix: true,
                                      })}
                                    </>
                                  )}
